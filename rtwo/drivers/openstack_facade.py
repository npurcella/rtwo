...
class OpenStack_Esh_Connection(OpenStack_1_1_Connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(OpenStack_Esh_Connection, self).__init__(*args,
                                                       timeout=timeout,
                                                       **kwargs)
    ...
    def request(self,
        ...
        while current_attempt < max_attempts:
            try:
                current_attempt += 1
                response = super(OpenStack_1_1_Connection,
...
class OpenStack_Esh_NodeDriver(OpenStack_1_1_NodeDriver):
    ...
    features = {
        ...
        "_to_node": [
            "Build public_ips field", "public_ips as extra",
            "keypairs as extra", "user/tenant as extra"
        ],
        "create_node":
        ["Create node with ssh_key", "ssh_key", "availability_zone"],
        "ex_create_node_with_network":
        ["Create node with floating IP"
         " and ssh_key", "ssh_key"],
        ...
        "ex_list_all_volumes":
        ["List all volumes for all tenants"
         " for the user"],
    ...
    def _to_snapshot(self, api_ss):
        ...
        extra = {
            'volume_id':
            api_ss.get('volumeId', api_ss.get('volume_id')),
            'name':
            api_ss.get('displayName', api_ss.get('display_name')),
            'created':
            api_ss.get('createdAt', api_ss.get('created_at')),
            'description':
            api_ss.get('displayDescription',
                       api_ss.get('display_description')),
            'status':
            api_ss['status']
        }
        snapshot = VolumeSnapshot(id=api_ss['id'],
                                  driver=self,
                                  size=api_ss['size'],
                                  extra=extra)
        return snapshot
    ...
    def _to_volumes(self, el, cinder=False):
        return [
            self._to_volume(volume, cinder=cinder) for volume in el['volumes']
        ]

    def _cinder_volume_args(self, api_volume):
        api_volume['createdAt'] = api_volume.pop('created_at')
        api_volume['displayName'] = api_volume.pop(
            'name', api_volume.pop('display_name', '<No Name>'))
        api_volume['displayDescription'] = api_volume.pop(
            'description', api_volume.pop('display_description', ''))
    ...
    def _to_image(self, api_machine):
        ...
        image.extra['image_size'] = api_machine[
            'OS-EXT-IMG-SIZE:size']  # NOTE: This may change. keep an eye on this.. -Steve
    ...
    def _keystone_get_tenant(self, tenant_id_or_name):
        ...
        for tenant in all_tenants:
            if tenant['id'] == tenant_id_or_name or tenant[
                    'name'] == tenant_id_or_name:
                return tenant
        return None
    ...
    def ex_deploy_to_node(self, node, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                "Ip Address found after calling wait_until_running: %s" %
                ip_addresses)
        except Exception:
            e = sys.exc_info()[1]
        ...
        for deploy_step in deploy_steps:
            if deploy_step.exit_status != 0:
                failed_steps.append("Script:%s returned a Non-Zero exit"
    ...
    def _connect_and_run_deployment_script(self, task, node, ssh_hostname,
        ...
        self._run_deployment_script(task=task,
                                    node=node,
    ...
    def ex_get_tenant_network(self, tenant_id):
        ...
        tenant_network = [
            net for net in networks if tenant_id == net.tenant_id
        ]
    ...
    def ex_vnc_console(self, node, vnc_type='novnc'):
        ...
        * xvpvnc (For Java Clients)
        """
        resp = self._node_action(node, 'os-getVNCConsole', type=vnc_type)
    ...
    def ex_list_snapshots(self):
        ...
        """
        server_resp = self.connection.request('/os-snapshots')
        return [
            self._to_snapshot(snapshot)
    ...
    def ex_create_snapshot(self,
        ...
        server_resp = self.connection.request('/snapshots',
                                              data=body,
                                              method='POST')
    ...
    def ex_get_snapshot(self, snapshot_id):
        server_resp = self.connection.request('/os-snapshots/%s' % snapshot_id)
        return self._to_snapshot(server_resp.object)
    ...
    def ex_boot_volume(self, **kwargs):
        ...
        if not kwargs.get('volume') and not kwargs.get('volume_size'):
            raise ValueError("Conflict: Must define an explicit volume_size "
    ...
    def create_volume(self,
        ...
        server_resp = self.connection.request(
            '/volumes',  # v3 doesnt use os-volumes anymore
            method='POST',
            data=body,
            **connection_kwargs)
    ...
    def ex_list_all_sizes(self):
        ...
        """
        server_resp = self.connection.request('/flavors/detail?all_tenants=1',
                                              method='GET')
    ...
    def ex_list_all_instances(self):
        ...
        while True:
            response = self.connection.request("/servers/detail?" +
                                               query_params)
            ...
            query_params = build_query_params(all_tenants,
                                              limit,
                                              marker=last_server["id"])
    ...
    def _neutron_delete_quota(self, tenant_id):
        ...
        server_resp = self.connection.request('/v2.0/quotas/%s.json' %
                                              tenant_id,
                                              method='DELETE')
    ...
    def _neutron_show_quota(self, tenant_id=None):
        ...
        if not tenant_id:
            raise Exception("Error calling /v2.0/quotas/tenant.json - %s" %
                            tenant_obj)
        # call to show quota:
        resp = self.connection.request('/v2.0/quotas/%s.json' % tenant_id)
        quota = resp.object['quota']
    ...
    def _cinder_delete_quota(self, username):
        ...
        server_resp = lc_conn.request('/os-quota-sets/%s' % username,
                                      method='DELETE')
        return server_resp.status == 204
    ...
    def ex_list_all_volumes(self):
        lc_conn = self.connection
        server_resp = lc_conn.request('/volumes/detail?all_tenants=1',
                                      method='GET')
    ...
    def ex_update_volume_metadata(self, volume, metadata):
        ...
        server_resp = self.connection.request('/volumes/%s/metadata' %
                                              volume.id,
                                              method='PUT',
                                              data=data_dict)
        try:
    ...
    def ex_delete_volume_metadata_key(self, volume, metadata_key):
        """
        """
        server_resp = self.connection.request('/volumes/%s/metadata/%s' %
                                              (volume.id, metadata_key),
                                              method='DELETE')
        try:
    ...
    def ex_list_volume_attachments(self, node):
        ...
        server_id = node.id
        server_resp = self.connection.request(
            '/servers/%s/os-volume_attachments' % server_id, method='GET')
    ...
    def ex_lookup_hypervisor_id_by_name(self, hypervisor_name):
        matches = [
            hv['id'] for hv in self.ex_list_hypervisor_nodes()
    ...
    def ex_list_instances_on_node(self, node_id):
        ...
        return self.connection.request("/os-hypervisors/%s/servers" %
                                       node_id).object['hypervisors']

    def ex_list_hypervisor_nodes(self):
        return self.connection.request("/os-hypervisors").object['hypervisors']
    ...
    def ex_detail_hypervisor_node(self, node_id):
        if type(node_id) == str:
            node_id = self.ex_lookup_hypervisor_id_by_name(node_id)
        return self.connection.request("/os-hypervisors/%s" %
                                       node_id).object['hypervisor']
    ...
    def ex_allocate_floating_ip(self, pool_name, **kwargs):
        ...
        """
        try:
            floating_ip_obj = self.connection.request('/os-floating-ips',
                                                      method='POST',
                                                      data={
                                                          'pool': pool_name
                                                      }).object
    ...
    def ex_deallocate_floating_ip(self, floating_ip, **kwargs):
        ...
        """
        try:
            server_resp = self.connection.request('/os-floating-ips/%s' %
                                                  floating_ip,
                                                  method='DELETE')
            return server_resp.object
        except Exception as e:
    ...
    def ex_bulk_delete_floating_ips(self, ip_range):
        ...
        try:
            ...
            server_resp = self.connection.request(
                '/os-floating-ips-bulk/delete', method='POST', data=data)
            return server_resp.object
        except Exception as e:
            raise

    def ex_bulk_create_floating_ips(self,
                                    ip_range,
                                    pool="nova",
                                    interface="eth0"):
        """
        ...
        try:
            ...
            server_resp = self.connection.request('/os-floating-ips-bulk',
                                                  method='POST',
                                                  data=data)
            return server_resp.object
        except Exception as e:
    ...
    def ex_disassociate_floating_ip(self, server, address=None, **kwargs):
        ...
        try:
            ...
            if not address:
                logger.warn("No public IP address found for instance %s",
                            server.id)
                return None
            server_resp = self.connection.request(
                '/servers/%s/action' % server.id,
    ...
    def ex_attach_interface(self, instance_id, network_id=None, port_id=None):
        ...
        server_resp = self.connection.request(
            uri, method="POST", data={"interfaceAttachment": attach_data})
    ...
    def ex_delete_metadata(self, node, key):
        ...
        @rtype: C{bool}
        """
        resp = self.connection.request('/servers/%s/metadata/%s' % (
            node.id,
            key,
        ),
                                       method='DELETE')
    ...
    def ex_set_image_metadata(self, image, metadata):
        ...
        @rtype: C{dict}
        """
        return self.connection.request('/images/%s/metadata' % (image.id, ),
                                       method='POST',
                                       data={
                                           'metadata': metadata
                                       }).object['metadata']
    ...
    def ex_delete_image_metadata(self, image, key):
        ...
        @rtype: C{bool}
        """
        resp = self.connection.request('/images/%s/metadata/%s' % (
            image.id,
            key,
        ),
                                       method='DELETE')
    ...
    def ex_shelve_instance(self, server, **kwargs):
        try:
            server_resp = self.connection.request('/servers/%s/action' %
                                                  server.id,
                                                  method='POST',
                                                  data={'shelve': None})
            return server_resp.object
    ...
    def ex_update_quota_for_user(self,
                                 tenant_id,
                                 user_id,
                                 values,
                                 use_tenant_id=True):
