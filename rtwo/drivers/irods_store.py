...
class IRODSDriver(StorageDriver):
    ...
    def __init__(self,
        ...
        self.connection = self.connectionCls(
            *args, **self._ex_connection_class_kwargs())
    ...
    def iterate_containers(self):
        ...
        """
        base_container = self.connection.session.collections.get(
            self.base_path)
        if not base_container:
            raise LibcloudError(
                'Failed to retrieve containers from base container:%s' %
                self.base_path,
                driver=self)
    ...
    def create_object(self, container, object_name):
        ...
        try:
            new_data_object = self.connection.session.data_objects.create(
                object_path)
    ...
    def upload_object(self,
        ...
        try:
            ...
            #Create the object
            data_object = self.connection.session.data_objects.create(
                object_path)
    ...
    def _get_objects(self, container):
        ...
        for root, collections, data_objects in self.connection.walk(
                container_url):
